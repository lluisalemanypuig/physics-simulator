...
namespace study_cases {
	...
	void print_geogebra_trajectory(const vector<vec3>& t, ofstream& fout) {
		fout << "{";
	...
	void mesh1d_case(int argc, char *argv[]) {
		...
		if (output == "none") {
			...
			if (not fout.is_open()) {
				...
				print_geogebra_trajectory(t1, fout);
				print_geogebra_trajectory(t2, fout);
				print_geogebra_trajectory(t3, fout);
				print_geogebra_trajectory(t4, fout);
			}
		}
	}
